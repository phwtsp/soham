...
class BreathingNotifier extends Notifier<BreathingState> {
  ...
  BreathingState build() {
    ...
    _initTimer(initialState.duration);

    return initialState;
  }

  void _initTimer(Duration duration) {
    _timer = Timer(duration, _updatePhase);
  }

  void _updatePhase() {
    // Como estamos dentro de um callback assíncrono, 'state' já está disponível.
    // O timer já disparou, então mudamos para a próxima fase.
    ...
    // Agenda a próxima mudança baseada na duração do novo estado
    _initTimer(state.duration);
  }
}

final breathingProvider =
    NotifierProvider.autoDispose<BreathingNotifier, BreathingState>(
